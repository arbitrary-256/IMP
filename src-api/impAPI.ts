...
// place routes between these comments
...
impAPI.get(`/`, (request: Request, response: Response) => response.status(200).json({}).send(`root`))
impAPI.get(`/about`, (request: Request, response: Response) => response.status(200).json({}).send(`about`))
impAPI.get(`/hello`, (request: Request, response: Response) => response.status(200).json({ message: `Hello ${request.query.name}` }))
// place routes between these comments

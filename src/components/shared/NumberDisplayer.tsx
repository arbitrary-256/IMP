...
export const NumberDisplayer: React.FC<IIMPNumber> = (impNumber: IIMPNumber): React.ReactElement => (
  <>{`${impNumber.id}: ${impNumber.prefix ? impNumber.prefix : ``} ${impNumber.id.endsWith(`Price`) ? impNumber.value.toFixed(2) : impNumber.value} ${impNumber.suffix ? impNumber.suffix : ``}`}</>
)
